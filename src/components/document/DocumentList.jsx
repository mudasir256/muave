...
if (myState?.userData.role === 'owner') {
  filteredDocuments = myState?.organization
  ...
  filteredDocuments = myState?.userData.Document
  .filter((document) => {
    const fileNameLower = document.fileName.toLowerCase();
    const clientNameLower = myState.userData.fullName.toLowerCase();
    const categoryLower = document.Category.toLowerCase();
    const statusLower = document.status.toLowerCase();
    const expireDateLower = document.expireDate.toLowerCase();
    const filterLower = filterValue.toLowerCase();
      ...
      <table className="table-auto w-full border-separate border-spacing-y-[40px] border-spacing-x-[10px]">
        <thead>
          <tr className="border">
            ...
            {myState.userData.role === 'owner' && <th className="text-start text-[#8A8A8A] font-medium">Client Name</th>}
        ...
        <tbody>
          ...
          {filteredDocuments?.map((document, index) => {
            let organization;
            if (myState.userData.role === 'client') { 
              organization = myState.userData
            } else {
              organization = myState.organization.find(
            ...
            return (
              ...
              <tr key={index}>
                ...
                {myState.userData.role === 'owner' && <td>{organization?.fullName || "No Name"}</td>}
                <td className="">
